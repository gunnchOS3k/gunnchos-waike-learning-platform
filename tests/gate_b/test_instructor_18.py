...
def test_instructor_section_and_grading(client, packs_18, track_id):
    ...
    # Grading from THIS track's packaged assignment — never DIGITAL_CONFIDENCE seed stand-in.
    if counts["assignments"] == 0 and counts["rubrics"] == 0:
        assert (registered.get("status") or {}).get("assignments") == "NOT_APPLICABLE"
        assert registered.get("assignment") is None
        return

    if counts["assignments"] == 0:
        # Rubrics exist without assignments: honest N/A for assignment grading path.
        assert registered.get("assignment") is None
        return
    ...
    assert assignment["module_id"] == track_id
    assert (assignment.get("rubric") or {}).get("criteria"), "pack registration must attach rubric"
