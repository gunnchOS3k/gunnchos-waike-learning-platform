.PHONY: bootstrap lint test build verify-pr1 verify-pr2 compile-dc rust-test frontend-test hub-test python-test assessment-test clean
...
python-test:
	WAIKE_ROOT=$(WAIKE_ROOT) $(PYTHON) -m pytest -q tests services/hub/tests

assessment-test:
	WAIKE_ROOT=$(WAIKE_ROOT) $(PYTHON) -m pytest -q services/hub/tests tests/assessment

hub-test:
	WAIKE_ROOT=$(WAIKE_ROOT) $(PYTHON) -m pytest -q services/hub/tests
...
# PR2 assessment lifecycle acceptance.
